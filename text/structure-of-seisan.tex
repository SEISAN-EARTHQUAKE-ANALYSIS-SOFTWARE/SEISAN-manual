...
Each event contains original phase readings in the 
...
The monthly epicenter files are called texttt{199901.CAT} for e.g. January 
...
\newline
We are working on implementing archive reading in SEISAN and a first version is now being tested. The reading routines using Chad Trabant software have been implemented by Ruben Luis.
...
When a keyword for archive type (BUD or SCP) is found, the reading is directed to the archive instead of to a file. The archive reference is e.g.
\begin{verbatim}
   ARC STAT  COM NT LO YYYY MMDD HHMM SS   DUR
   BUD ROSA  BHZ PM    2010 1011 0100 00 14400                                   6
\end{verbatim}
where ARC is archive type, STAT is station code, COM is component, LO is location code YYYY MMDD HHMM SS is start time and DUR is duration in secs.
\newline
...
\end{verbatim}
where each channel is defined as well as the location of the archive. The specification is the same for both BUD and SeisComp archives, and only when reading S-files or starting mulplt will the information about archive types be given. ONLY one archive type can be used at the same time when, maybe something to change.
\newline
Current it has only been tested on Linux but the plan is to also make it work on Windows.
...
be stored in SEISAN, GSE and SAC format, see section 
